...
def create_pred_format(
    preds_format_path,
    suffix,
    dt
) -> None:
    ### Creating the pred format csv file
    pred_file = pd.DataFrame(index = dt.index, columns = ['Score']) #.reset_index(names = ['MessageId'])
    pred_file['Score'] = 0
    pred_file.to_csv(preds_format_path + '/' + suffix + '_pred_format.csv', index=False)
...
def freq_by_features (
    ...
    if map_from_train_set == True:
        ...
        level_1_level_2_frequency = pickle.load(open(model_dir + '/' + str(level_1 + "_" + level_2 + "_frequency")  + '.sav', 'rb'))
        ...
        df.loc[:, str(level_1 + "_" + level_2 + "_frequency")] = \
                    df.loc[:,str(level_1 + "_" + level_2)].map(level_1_level_2_frequency)
    else:
        _level_1 = df.loc[:,level_1].unique()
        _level_2 = df.loc[:,level_2].unique()
        df[str(level_1 + "_" + level_2)] = df.loc[:,level_1] + df.loc[:,level_2].astype(str)
        ...
        for s in _level_1:
            ...
            for h in _level_2:
                level_1_level_2_frequency[s + str(h)] = len(level_1_rows[level_1_rows.loc[:,level_2] == h])

        df.loc[:, str(level_1 + "_" + level_2 + "_frequency")] = \
                    df.loc[:,str(level_1 + "_" + level_2)].map(level_1_level_2_frequency)

        if save_as_object == True:
            pickle.dump(level_1_level_2_frequency, open(model_dir + '/' + str(level_1 + "_" + level_2 + "_frequency")  + '.sav', 'wb'))
...
def create_features(
    ...
    freq_by_features(df = df, level_1="Sender" , level_2="hour", model_dir = model_dir, save_as_object = True, map_from_train_set=map_from_train_set)
    # Hour frequency for each receiver
    freq_by_features(df = df, level_1="Receiver" , level_2="hour", model_dir = model_dir, save_as_object = True, map_from_train_set=map_from_train_set)
    # Sender-Currency Frequency
    freq_by_features(df = df, level_1="Sender" , level_2="InstructedCurrency",model_dir = model_dir, save_as_object = True, map_from_train_set=map_from_train_set)
    # Receiver-SettledCurrency Frequency
    freq_by_features(df = df, level_1="Receiver" , level_2="SettlementCurrency",model_dir = model_dir, save_as_object = True, map_from_train_set=map_from_train_set)
    # Sender-Receiver Frequency
    freq_by_features(df = df, level_1="Sender" , level_2="Receiver",model_dir = model_dir, save_as_object = True, map_from_train_set=map_from_train_set)
    ...
    df = df.sort_values(by = ['SettlementDate','Sender', 'Receiver','Account_order', 'Account_ben', 'Timestamp'], ascending=True)
    df['seq'] = df.groupby(['SettlementDate','Sender', 'Receiver','Account_order', 'Account_ben']).cumcount() + 1
    ...
    df['DifferentOrderNum'] = np.where(df['Account_order'] == df['OrderingAccount'],0,1)
    # Different receiver account number from bank details
    df['DifferentBenNum'] = np.where(df['Account_ben'] == df['BeneficiaryAccount'],0,1)

    # Different sender account name from bank details
    df['DifferentOrderName'] = np.where(df['Name_order'] == df['OrderingName'],0,1)
    # Different receiver account name from bank details
    df['DifferentBenName'] = np.where(df['Name_ben'] == df['BeneficiaryName'],0,1)

    # Different sender account ordering street from bank details
    df['DifferentOrderStreet'] = np.where(df['Street_order'] == df['OrderingStreet'],0,1)
    # Different receiver account ordering street from bank details
    df['DifferentBenStreet'] = np.where(df['Street_ben'] == df['BeneficiaryStreet'],0,1)

    # Different sender account country code/zip from bank details
    df['DifferentOrderZip'] = np.where(df['CountryCityZip_order'] == df['OrderingCountryCityZip'],0,1)
    # Different receiver account country code/zip from bank details
    df['DifferentBenZip'] = np.where(df['CountryCityZip_ben'] == df['BeneficiaryCountryCityZip'],0,1)
...
#     print("AUPRC:", metrics.average_precision_score(y_true=Y_train_data, y_score=pred_proba))
    print(metrics.average_precision_score(y_true=Y_train_data, y_score=pred_proba))

    importances = model.feature_importances_
    forest_importances = pd.Series(importances, index=train_data.drop(["Label"], axis=1).columns)
...
def fit(
    ...
    train_data = pd.merge(train_data, bank_data, left_on = "BeneficiaryAccount", right_on = "Account", how = "left",suffixes = ['_order','_ben'])

    # Creating predictions format file
    create_pred_format(preds_format_path= preds_format_path, dt=train_data, suffix = "train")

    # Feature engineering

    train_data = create_features(df=train_data, model_dir=model_dir, map_from_train_set=False)
...
def predict(
    ...
    # Creating predictions format file
    create_pred_format(preds_format_path = preds_format_path, dt = train_data, suffix = "test")

    # Feature engineering

    train_data = create_features(df=train_data, model_dir=model_dir, map_from_train_set=True)
...
#     print(train_data_2.head(3))
    ...
    scaler = pickle.load(open(model_dir + '/finalized_scaler_' + m + '.sav','rb'))
    X_train_data = scaler.transform(X_train_data)
